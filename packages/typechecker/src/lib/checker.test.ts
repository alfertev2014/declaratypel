...
describe("inferType", () => {
  ...
  describe("of object template", () => {
    ...
    test("with props of literal types", (t) => {
      t.test(
        "with one prop should return object type with this prop and literal type as value type",
        () => {
          const inferred = inferType(
            obj([propValue("foo", literal("bar"))]),
            null,
          )
          assert.strictEqual(inferred.tctor, OBJECT)
      ...
      t.test(
        "with two props should return object type with these props in the same order and literal types as value types",
        () => {
          const inferred = inferType(
            obj([
              propValue("foo1", literal("bar1")),
              propValue("foo2", literal("bar2")),
            ]),
            null,
          )
          assert.strictEqual(inferred.tctor, OBJECT)
